package com.example.mesher.platform.wifidirect

import java.io.InputStream
import java.net.ServerSocket
import java.net.Socket
import kotlin.concurrent.thread

/**
 * TCP server running on the WiFi Direct group owner (port 7891).
 * Non-GO devices connect here after the P2P group is formed.
 */
class WifiDirectServer(
    private val onReceive: (nodeId: String, data: ByteArray) -> Unit
) {
    companion object { const val PORT = 7891 }

    private var serverSocket: ServerSocket? = null
    private val clients = mutableMapOf<String, Socket>()
    private var running = false

    fun start() {
        if (running) return
        running = true
        thread(isDaemon = true, name = "wifidirect-server") {
            try {
                val ss = ServerSocket(PORT).also { serverSocket = it }
                while (running) {
                    val client = ss.accept()
                    handleClient(client)
                }
            } catch (_: Exception) {}
        }
    }

    private fun handleClient(socket: Socket) {
        thread(isDaemon = true, name = "wifidirect-client") {
            try {
                val input: InputStream = socket.getInputStream()
                // First 8 bytes = nodeId hex (ASCII)
                val idBuf = ByteArray(8)
                var read = 0
                while (read < 8) {
                    val n = input.read(idBuf, read, 8 - read)
                    if (n < 0) return@thread
                    read += n
                }
                val nodeId = idBuf.joinToString("") { "%02X".format(it) }
                clients[nodeId] = socket
                // Read length-prefixed frames
                val lenBuf = ByteArray(4)
                while (running && !socket.isClosed) {
                    if (input.read(lenBuf) != 4) break
                    val len = ((lenBuf[0].toInt() and 0xFF) shl 24) or
                              ((lenBuf[1].toInt() and 0xFF) shl 16) or
                              ((lenBuf[2].toInt() and 0xFF) shl 8) or
                              (lenBuf[3].toInt() and 0xFF)
                    if (len <= 0 || len > 4_000_000) break
                    val data = ByteArray(len)
                    var got = 0
                    while (got < len) {
                        val n = input.read(data, got, len - got)
                        if (n < 0) break
                        got += n
                    }
                    onReceive(nodeId, data)
                }
            } catch (_: Exception) {
            } finally {
                try { socket.close() } catch (_: Exception) {}
            }
        }
    }

    fun send(nodeId: String, data: ByteArray) {
        val socket = clients[nodeId] ?: return
        try {
            val out = socket.getOutputStream()
            val lenBytes = byteArrayOf(
                (data.size shr 24).toByte(), (data.size shr 16).toByte(),
                (data.size shr 8).toByte(), data.size.toByte()
            )
            synchronized(out) {
                out.write(lenBytes)
                out.write(data)
                out.flush()
            }
        } catch (_: Exception) {
            clients.remove(nodeId)
        }
    }

    fun stop() {
        running = false
        clients.values.forEach { try { it.close() } catch (_: Exception) {} }
        clients.clear()
        try { serverSocket?.close() } catch (_: Exception) {}
        serverSocket = null
    }
}
